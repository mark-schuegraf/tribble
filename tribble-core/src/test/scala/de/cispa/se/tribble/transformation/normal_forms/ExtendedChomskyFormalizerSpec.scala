...
class ExtendedChomskyFormalizerSpec extends TransformationTestSpecification {

  "Extended Chomsky formalizer" should "eliminate terminal symbols except in rules of size 1 (TERM)" in {
    val g = Grammar(
      'S := "s1" ~ "s2",
    )
    val eG = Grammar(
    ...
    checkGrammarEquality(ExtendedChomskyFormalizer, g, eG)
  }
  ...
  it should "not eliminate unit rules" in {
    val g = Grammar(
      'S := 'A,
      'A := "a1" | 'B,
      'B := "b1" | 'A
  ...
  it should "transform a simple arithmetic expression grammar correctly" in {
    ...
    val gTerm = Grammar(
      'S := 'D | 'I,
      'D := "",
      'I := "a" | "b" | 'I_term0 | 'I ~ 'I_term1 | 'I ~ 'I_term2 | 'I ~ 'I_term3,
      'I_term0 := "0",
      'I_term1 := "a",
      'I_term2 := "b",
      'I_term3 := "1",
